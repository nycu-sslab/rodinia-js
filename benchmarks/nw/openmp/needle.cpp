...
////////////////////////////////////////////////////////////////////////////////
void runTest(int argc, char **argv) {
  int max_rows, max_cols, penalty, row_col_size;
  ...
  char filename[50];
  sprintf(filename, "../inputGen/%d.txt", row_col_size);
  ...
  for (int i = 1; i < max_rows; i++) { // please define your own sequence.
    fscanf(pFile, "%d", &tmp);
    input_itemsets[i * max_cols] = tmp;
  }
  for (int j = 1; j < max_cols; j++) { // please define your own sequence.
    fscanf(pFile, "%d", &tmp);
    input_itemsets[j] = tmp;
  }
